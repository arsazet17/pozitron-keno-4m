...
const $=id=>document.getElementById(id);let DATA={draws:[],frozen:{records:[]},meta:{},combos:{}};let comboCol=1;
const fmt=n=>String(n).padStart(2,'0');

/*
  СКРЫТАЯ ТАБЛИЦА ВЫПЛАТ КЕНО.
  Формат: PAYOUTS[сколько выбрано][сколько угадано] = рублей.
  В интерфейсе сама таблица нигде не показывается.
*/
const PAYOUTS={
...
function comboPrize(size,hits){
  const n=Number(size),h=Number(hits);
  return Number(PAYOUTS[n]?.[h]||0);
}
function rub(n){return `${Number(n).toLocaleString('ru-RU')} ₽`}
function toast(t){const e=$('toast');e.textContent=t;e.classList.add('show');setTimeout(()=>e.classList.remove('show'),1800)}
async function j(path){const r=await fetch(path+'?t='+Date.now(),{cache:'no-store'});if(!r.ok)throw new Error(path);return r.json()}
function go(p){document.querySelectorAll('.page').forEach(x=>x.classList.toggle('active',x.dataset.page===p));document.querySelectorAll('.nav').forEach(x=>x.classList.toggle('active',x.dataset.go===p));scrollTo(0,0)}
document.addEventListener('click',e=>{const b=e.target.closest('[data-go]');if(b)go(b.dataset.go)});
function latest(){return DATA.draws.at(-1)}function openFrozen(){return [...DATA.frozen.records].reverse().find(r=>!r.closed)}
function renderLatest(){const d=latest();if(!d)return; $('latestHead').textContent=`№${d.draw} · ${d.date} · ${d.time}`;$('latestColumn').textContent=d.column;$('latestBalls').innerHTML=d.balls.map(n=>`<span class="ball ${(((n-1)%10)+1)==d.column?'winner':''}">${fmt(n)}</span>`).join('');const a=d.audit||{};const counts=a.counts||{};$('auditBox').innerHTML=`<div class="audit-grid">${Array.from({length:10},(_,i)=>`<div class="audit-cell">ст${i+1}<b>${counts[String(i+1)]??'—'}</b></div>`).join('')}</div><p class="muted">${(a.tied||[]).length>1?`Ничья: ${(a.tied||[]).map(c=>`ст${c}→поз.${a.completion?.[String(c)]}`).join(' · ')}. Corrected победитель: ст${a.calculated}.`:`Единственный лидер. Tie-break не требуется.`}</p>`}
function renderForecast(){const r=openFrozen();if(!r){$('forecastTarget').textContent='Открытого frozen нет';$('forecastRows').innerHTML='';return}$('forecastTarget').textContent=`На ${r.target.date} ${r.target.time} · №${r.target.draw}`;$('nextDraw').textContent=`№${r.target.draw} · ${r.target.time}`;$('forecastRows').innerHTML=(r.options||[]).map((o,i)=>`<div class="frow ${i===0?'main':''}"><div class="flabel">${o.label}</div><div class="fcol">ст${o.column}</div><div class="fmeta"><b>SCORE ${Number(o.score||0).toLocaleString('ru-RU')}</b><div class="combochips">${(o.combo||[]).map(n=>`<span class="chip">${fmt(n)}</span>`).join('')}</div>${o.specificity!=null?`<div class="spec">support ${o.support} / ALL ${o.all} · P ${(o.specificity*100).toFixed(3)}%</div>`:''}</div></div>`).join('');$('rankingBox').innerHTML=(r.ranking||[]).map((x,i)=>`<div class="rankrow"><b>${i+1}</b><span>ст${x[0]}</span><b>${Number(x[1]).toLocaleString('ru-RU')}</b></div>`).join('')}
function renderDraws(){const arr=[...DATA.draws].reverse().slice(0,50);$('drawList').innerHTML=arr.map(d=>`<details class="draw-card"><summary><div><div class="dh-title">№${d.draw}</div><div class="dh-meta">${d.date} · ${d.time}</div></div><div class="colbadge">ст${d.column}</div></summary><div class="detail"><div class="balls20" style="margin-top:12px">${d.balls.map(n=>`<span class="ball ${(((n-1)%10)+1)==d.column?'winner':''}">${fmt(n)}</span>`).join('')}</div><p class="muted">${(d.audit?.tied||[]).length>1?`Tie: ${(d.audit.tied||[]).map(c=>`ст${c}→${d.audit.completion?.[String(c)]}`).join(' · ')}`:'Tie-break не требовался'}</p></div></details>`).join('')}
function clsText(r){if(r.classification==='V1')return['✅ В1','hit'];if(r.classification==='V2'||r.classification==='V3')return[`☑️ ${r.classification}`,'hit'];if(r.classification==='reserve-hit')return['🛟 резерв','reserve'];return['❌ мимо','miss']}
...
function renderCombos(){const tabs=Array.from({length:10},(_,i)=>i+1);$('comboTabs').innerHTML=tabs.map(c=>`<button class="coltab ${c===comboCol?'active':''}" data-col="${c}">ст${c}</button>`).join('');const arr=DATA.combos[String(comboCol)]||[];$('comboList').innerHTML=arr.slice(0,20).map((x,i)=>`<div class="combo-row"><strong>${i+1}. ${x.combo.map(fmt).join(' · ')}</strong><div class="combo-stat"><span>размер ${x.size}</span><span>support <b>${x.support}</b></span><span>ALL ${x.all}</span><span>P <b>${(x.specificity*100).toFixed(3)}%</b></span></div></div>`).join('')}
document.addEventListener('click',e=>{const b=e.target.closest('[data-col]');if(b){comboCol=Number(b.dataset.col);renderCombos()}});
...
if('serviceWorker'in navigator)window.addEventListener('load',async()=>{
 try{
  const reg=await navigator.serviceWorker.register('./sw.js?v=1.0.4',{updateViaCache:'none'});
  await reg.update();
  let reloading=false;
