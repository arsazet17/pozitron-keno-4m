...
async def collect(page):
 ...
 return rows.map(el=>{const chunks=[];const add=n=>{if(n){const t=norm(n.innerText||n.textContent);if(t)chunks.push(t)}};add(el);let p=el.parentElement;for(let i=0;p&&i<4;i++,p=p.parentElement){add(p);if(/столб/i.test(chunks.join(' ')))break;}add(el.previousElementSibling);add(el.nextElementSibling);const btn=[...el.querySelectorAll('button')].map(x=>norm(x.innerText||x.textContent));const atoms=[...el.querySelectorAll('[class*="ball" i],[class*="number" i],[class*="win" i]')].map(x=>norm(x.innerText||x.textContent));return {text:norm(el.innerText),context:chunks.join('\n'),dateLabel:dateBefore(el),buttons:btn,atoms};});
}""")
    out=[]; carry=''
    for row in raw:
        ...
        if not col: raise RuntimeError(f'№{draw}: не найден официальный столб')
        balls=[]
        for pool in (row.get('buttons') or [],row.get('atoms') or []):
            nums=[]
            for x in pool:
                if re.fullmatch(r'0?([1-9]|[1-7]\d|80)',norm(x)): nums.append(int(x))
            if len(nums)>=20: balls=nums[:20]; break
        if len(balls)!=20: raise RuntimeError(f'№{draw}: найдено {len(balls)} чисел вместо 20')
        if len(set(balls))!=20: raise RuntimeError(f'№{draw}: 20 чисел содержат дубли')
        d={'draw':draw,'date':ds,'time':tm,'column':col,'balls':balls,'source':'Официальный Столото OAuth · FULL20 stable-v1.2'}
        d['audit']=audit(d)
...
def snapshot_key(arr):
    return tuple((d['draw'],d['date'],d['time'],d['column'],tuple(d['balls'])) for d in arr)

async def get_stable_tail(page,email,password,known_last_draw):
    snapshots=[]; keys=[]; latest_by_key={}; counts=Counter(); bad=0; zero_streak=0
    for i in range(MAX_READS):
        try:
            arr=await collect(page)
            if len(arr)<TAIL:
                bad+=1; zero_streak=zero_streak+1 if not arr else 0
                print(f'Чтение {i+1}: неполный архив {len(arr)}/{TAIL} — повторяю')
                if auth_url(page.url):
                    print('Архив вернул OAuth — повторный вход'); await login(page,email,password)
                if zero_streak>=2 and not auth_url(page.url):
                    print('Два пустых чтения — обновляю OAuth-сессию'); await page.context.clear_cookies(); await login(page,email,password); zero_streak=0
                if i<MAX_READS-1: await page.wait_for_timeout(READ_DELAY_MS)
                continue
            zero_streak=0; latest_draw=arr[-1]['draw']
            if latest_draw < known_last_draw:
                bad+=1; print(f'Чтение {i+1}: старый хвост №{latest_draw} < сохранённого №{known_last_draw} — повторяю')
                if i<MAX_READS-1: await page.wait_for_timeout(READ_DELAY_MS)
                continue
            k=snapshot_key(arr); snapshots.append(arr); keys.append(k); latest_by_key[k]=latest_draw; counts[k]+=1
            print(f'Чтение {i+1}: №{arr[0]["draw"]}–№{latest_draw}; подтверждение {counts[k]}')
            if len(keys)>=3 and keys[-1]==keys[-2]==keys[-3]:
                print(f'STABLE PASS 3/3: №{latest_draw}'); return arr
            freshest=max(latest_by_key.values())
            candidates=[key for key,cnt in counts.items() if cnt>=2 and latest_by_key[key]==freshest]
            if candidates:
                chosen=candidates[-1]; idx=max(j for j,x in enumerate(keys) if x==chosen)
                print(f'SMART PASS 2/2: свежее состояние №{freshest}'); return snapshots[idx]
        except Exception as e:
            bad+=1; print(f'Чтение {i+1}: временный сбой {type(e).__name__}: {e}')
            try:
                if auth_url(page.url): print('После сбоя страница на OAuth — восстанавливаю вход'); await login(page,email,password)
            except Exception as le: print(f'Повторный OAuth пока не удался: {le}')
        if i<MAX_READS-1: await page.wait_for_timeout(READ_DELAY_MS)
    seen=[a[-1]['draw'] for a in snapshots if a]
    raise RuntimeError(f'FULL20 не стабилизирован за {MAX_READS} чтений; валидные latest={seen}; bad={bad}. Старые данные не изменены.')
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        context=await browser.new_context(locale='ru-RU',timezone_id='Europe/Moscow',viewport={'width':390,'height':844},user_agent='Mozilla/5.0 (Linux; Android 10) AppleWebKit/537.36 Chrome/131 Mobile Safari/537.36')
    ...
    STATUS.write_text(json.dumps({'updatedAt':datetime.now(timezone.utc).isoformat(),'stableDraws':TAIL,'latest':official[-1],'mode':'working-login-plus-stable-retry-v1.2'},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    print(f'FULL20 STOLOTO PASS v1.2: №{official[-1]["draw"]} {official[-1]["time"]} ст{official[-1]["column"]}')
