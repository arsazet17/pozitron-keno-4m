(function(global){
  ...
  function activeColumns(matrix,schedule=SCHEDULE){
    const hm=headerMap(matrix);
    return schedule.map(t=>hm[t]).filter(c=>c!=null);
  }
  ...
  // Закон: поиск продолжений только по активному расписанию проекта.
  function verticalSequences(matrix,schedule=SCHEDULE){
    const seqs=[], cols=activeColumns(matrix,schedule);
    for(const c of cols){
      const items=[];
  ...
  function horizontalSequences(matrix,schedule=SCHEDULE){
    const seqs=[], hm=headerMap(matrix);
    for(let r=1;r<matrix.length;r++){
      const items=[];
      for(const time of schedule){
        const c=hm[time]; if(c==null) continue;
  ...
  function variant2(matrix,specs,initialMethods){
    ...
    while(round<8){
      ...
      for(const [name,s] of Object.entries(specs)){
        ...
        next[name]=cur.usedLen>1 ? methodAtLen(matrix,s.chain,s.orientation,cur.usedLen-1) : cur;
      }
      current=next;
      round++;
    }
  ...
  function scannerValues(v1,v2,gg){
    const out=[...(v1||[])];
    if(v2!=null && !out.includes(v2)) out.push(v2);
    if(gg!=null && !out.includes(gg) && !(v1||[]).includes(gg)) out.push(gg);
    return out;
  }
  function predict(matrix,target){
    ...
    const scanner=scannerValues(v1.values,v2.value,gg.value);
    return {target,vChain,hChain,methods,v1,v2,gg,consensus,scanner};
  }
  ...
  global.KenoEngine={
    SCHEDULE,val,parseDate,formatDate,nextDate,cloneMatrix,headerMap,activeColumns,
    ensureDateRow,latestDateRow,nextTarget,verticalChain,horizontalChain,
    verticalSequences,horizontalSequences,findMethod,methodAtLen,variant1,variant2,
    extraGG,scannerValues,predict,applyOverrides
  };
